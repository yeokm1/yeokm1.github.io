scenario_4 = function () {
    ...
    function drawChart(altShare, altShareGRC, tableID, pieID, barChartTitle) {
        ...
        if (barChart == null) {
            barChart = new Chart(ctx1, {
                type: 'bar',
                data: {
                    labels: ["S-SMC1", "S-SMC2", "S-SMC3", "S-SMC4", "S-SMC5", "S-SMC6", "S-SMC7", "S-SMC8", "S-SMC9", "S-SMC10", "S-SMC11", "S-SMC12", "S-SMC13", "S-SMC14", "S-SMC15"],
                    datasets: [
                        {
                            label: "Alternative",
        ...
        // Singapore GRC
        tableData.rows[1].cells[1].innerHTML = (incShare.reduce((a, b) => a + b, 0) / 15).toFixed(1) + "%"
        tableData.rows[1].cells[2].innerHTML = (altShare.reduce((a, b) => a + b, 0) / 15).toFixed(1) + "%"
        ...
        tableData.rows[3].cells[1].innerHTML = (incShareTotal / altShare.length).toFixed(1) + "%"

        //Alternative popular vote %
        tableData.rows[3].cells[2].innerHTML = (altShareTotal / altShare.length).toFixed(1) + "%"

        //Popular Vote Swing
        tableData.rows[3].cells[0].innerHTML = "Popular Vote (Swing " + voteShift.toFixed(1) + "%)"
    ...
    function updateChart() {
        ...
        drawChart(altShareAfterCycle, altShareGRC, "scenario_4-bar", "scenario_4-pie", "Vote breakdown of individual SMCs")
    ...
    document.addEventListener("DOMContentLoaded", function (event) {
        ...
        document.getElementById("scenario_4-reset-button").addEventListener('click', resetButtonPressed);

        document.getElementById("scenario_4-randomise-button").addEventListener('click', randomiseButtonPressed);
